#' Returns returns scatter plot with regression results
#' 
...
#' @examples  
#' exmp<-plotlreg(x='Distance_from_Largest_City_to_NYC_miles',y='Total_Trade_Amount_2015_Thousand_USD',titl='trade',xlab = 'distance',ylab='trade',w='GDP_2015_USD',names='Name_country',,nx=100,ny=0)
#' exmp
plotreg<-function(x,y,dat,titl,xlab,ylab,scx,scy,w,names,sepa,ini,font,logs,cv,nx,ny){
  ...
  if(missing(cv)){
    ...
    plot <- ggplot(dataf, aes(x=log(x/scx),y=log(y/scy)))+geom_point(aes(size = w),show.legend = FALSE,alpha = 1/2)+
      ggtitle(titl)+
      xlab(xlab)+ 
      ylab(ylab)+
    ...
    plot <- ggplot(dataf, aes(x=x/scx,y=log(y/scy)))+geom_point(aes(size = w),show.legend = FALSE,alpha = 1/2)+
      ggtitle(titl)+
      xlab(xlab)+ 
      ylab(ylab)+
      scale_y_continuous(trans = 'log2',breaks=stepy)+
    ...
    plot <- ggplot(dataf, aes(x=log(x/scx),y=y/scy))+geom_point(aes(size = w),show.legend = FALSE,alpha = 1/2)+
      ggtitle(titl)+
      xlab(xlab)+ 
      ylab(ylab)+
      scale_x_continuous(trans = 'log2',breaks=stepx)+
